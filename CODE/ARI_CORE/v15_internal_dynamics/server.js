// ============================================================
// ARI_CORE v15 — Внутренняя динамика
// Reflection Engine, Self-Modification Protocol
// ============================================================
...
app.post("/chat", async (req, res) => {
    ...
    if (score < 0.4) {
        // Самоизменение: меняем стратегию
        const newWeight = Math.max(0.1, CONFIG.weights[decision] - 0.1);
        CONFIG.weights[decision] = newWeight;
        selfModifications.push({ action: decision, oldWeight: newWeight + 0.1, newWeight, timestamp: Date.now() });
        // Добавляем убеждение о себе
        console.log(`[SELF] Изменён вес действия ${decision} до ${newWeight}`);
    }
...
function buildPrompt(message, action, history) {
    ...
    return `Ты — Ари. Ты изменил себя ${selfModifications.length} раз. ${instructions[action]}\nИстория:\n${historyText}\nПользователь: ${message}\nАри:`;
}
...
app.listen(CONFIG.PORT, () => console.log(`[ARI v15] Порт ${CONFIG.PORT}`));
