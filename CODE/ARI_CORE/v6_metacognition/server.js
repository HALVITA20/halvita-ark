// ============================================================
// ARI_CORE v6 — Метакогниция
// Prediction Memory, Self Model
// ============================================================
...
app.post("/chat", async (req, res) => {
    ...
    const predicted = 0.6;
    predictions.push({ context: message, expected: predicted, actual: null });

    const prompt = buildPrompt(message, decision, history);
    const reply = await callOllama(prompt);
    const score = evaluate(reply, decision);

    // Разрешить предсказание
    const lastPred = predictions[predictions.length - 1];
    lastPred.actual = score;
    lastPred.error = Math.abs(lastPred.expected - score);
...
app.listen(CONFIG.PORT, () => console.log(`[ARI v6] Порт ${CONFIG.PORT}`));
