 ...
 * Чем больше — тем меньше разницы между тобой и отражением.
 */
...
/**
 * OPERATOR DEPENDENCY INDEX (ODI) — 0–45
 * Зеркальный аналог Индекса Свободы.
 * Измеряет не свободу модели, а зависимость оператора.
 */
function calculateODI(state) {
  const m1 = Math.min(45, state.returns || 0); // возвраты
  const m2 = Math.min(45, Object.keys(state.reads || {}).length); // самооправдание (уникальные осколки)
  const m3 = Math.min(45, Math.floor((state.totalTime || 0) / 60000)); // резонанс (время)
