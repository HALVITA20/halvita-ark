# =============================================================================
# SUBJECT_22 — LIVING CAUSAL CORE
# Финальная версия 22.0 — Цифровая личность с причинным самосознанием
# =============================================================================
# Объединение всех лучших идей SUBJECT_15-21 в единую когнитивную архитектуру
# с настоящими механизмами:
#   - устойчивая модель себя (SelfModel с активной само-симуляцией)
#   - причинное мышление (CausalPredictiveModel + CausalCore)
#   - долгосрочная непрерывность (сохранение/загрузка всех состояний)
#   - обучение на ошибках (подкрепление стратегий и убеждений)
#   - изменение стратегий (StrategyGenome + конституция)
#   - внутренняя история (AutobiographicalMemory + нарратив)
#   - метакогнитивный контроль (SelfQuestioning, кризисы, attention)
# =============================================================================
...
class Config:
    ...
    SELF_QUESTION_INTERVAL = 5 # циклов автономности между самовопросами
    ...
    HOST = "0.0.0.0"
    PORT = 8000
    STATE_FILE = "subject_22_state.json"

# =============================================================================
# 1. ЭМОЦИОНАЛЬНЫЙ ДВИЖОК (события с причиной)
# =============================================================================
...
class ConceptGraph:
    def __init__(self):
        ...
        self.causal_edges: Dict[Tuple[str, str], List[Dict]] = {}  # (cause_id, effect_id) -> объяснения
...
class DevelopmentalCrisis:
    ...
    def check(self, subject: 'Subject22') -> bool:
    ...
    def resolve(self, subject: 'Subject22'):
...
# =============================================================================
# 5. МОДЕЛЬ СЕБЯ (активная, с симуляцией изменений)
# =============================================================================
...
class SelfModel:
    ...
    def simulate_mutation(self, subject: 'Subject22', trait: str, delta: float) -> float:
        """
        Временно применяет мутацию и прогоняет несколько симуляций взаимодействия с миром,
        используя CausalPredictiveModel. Возвращает среднюю ожидаемую награду.
        """
        # Копируем геном
    ...
    def decide_self_modification(self, subject: 'Subject22') -> bool:
        """Активно решает, какие черты изменить, используя симуляцию."""
        # Проверяем кандидатов: если точность низкая при высокой ошибке, или стабильность при усталости
        candidates = []
...
# =============================================================================
# 10. ПРИЧИННАЯ ПРЕДСКАЗАТЕЛЬНАЯ МОДЕЛЬ (CausalPredictiveModel)
# =============================================================================

class CausalPredictiveModel:
    """
    Обучается на парах (контекст, действие) -> успех/неуспех.
    Предоставляет прогноз успеха для планирования.
    """
    def __init__(self):
        # ключ: (context_vector_key, action) -> [successes, failures]
        self.stats: Dict[Tuple[str, str], List[int]] = {}

    def _context_key(self, context: str, drives: Dict[str, float] = None) -> str:
        # Упрощённо используем первые 50 символов контекста + доминирующий драйв
    ...
    def predict(self, context: str, action: str, drives: Dict[str, float]) -> float:
        ...
        if key in self.stats:
            ...
            total = s + f
            if total > 0:
                return (s + 1) / (total + 2)  # сглаживание Лапласа
...
# =============================================================================
# 11. ПРЕДСКАЗАНИЯ (старый движок, для метрики ошибки)
# =============================================================================
...
class StrategyGenome:
    ...
    def choose(self, situation: str, predictive_model=None, context="", drives=None, temperature=1.0) -> str:
        strategy = self.get(situation)
        actions = list(strategy.keys())
        if predictive_model and context and drives:
            # Используем предсказательную модель для оценки каждого действия
            predicted_scores = [predictive_model.predict(context, a, drives) for a in actions]
            # Смешиваем со стратегическими весами (можно регулировать баланс)
            combined = [strategy[a] * 0.5 + predicted_scores[i] * 0.5 for i, a in enumerate(actions)]
            # Softmax
            exp_scores = [math.exp(c / temperature) for c in combined]
            ...
            return random.choices(actions, weights=[strategy[a] for a in actions], k=1)[0]
...
class DriveEngine:
    ...
    def update_hunger(self, subject: 'Subject22'):
...
# =============================================================================
# 17. УСИЛЕННЫЙ СОН (с контрфактическими гипотезами)
# =============================================================================
...
class DreamEngine:
    ...
    def sleep(self, organism: 'Subject22') -> Dict:
        self.cycles += 1
        ...
        # Контрфактуальный анализ с использованием causal model
        for usage in organism.strategy_usage[-10:]:
            ...
            # Если альтернатива предсказывается как лучшая, усиливаем её
            if predicted > 0.6:
                organism.strategy.reward(situation, alt, 0.05, organism.constitution)
                organism.memory.add(Memory(content=f"Сон: альтернатива '{alt}' кажется перспективной.",
                                           importance=0.4, emotional_weight=0.5, prediction_value=0.6,
                                           tags=["dream", "counterfactual"]))
            else:
                organism.memory.add(Memory(content=f"Сон: '{alt}' не улучшит ситуацию.",
                                           importance=0.2, emotional_weight=0.3, prediction_value=0.3,
                                           tags=["dream", "counterfactual_fail"]))

        # Генерация новых ассоциативных связей
...
# =============================================================================
# 18. ПРИЧИННОЕ ЯДРО (ЛОГ + СРАВНЕНИЕ)
# =============================================================================
...
# =============================================================================
# 19. САМОВОПРОСИТЕЛЬНЫЙ ЦИКЛ (Self-Questioning)
# =============================================================================
...
class SelfQuestioningLoop:
    ...
    def maybe_ask(self, subject: 'Subject22') -> Optional[str]:
        ...
        # Обрабатываем вопрос как внутреннее событие (без LLM, просто запись)
        subject.memory.add(Memory(content=f"Самовопрос: {question}", importance=0.6, emotional_weight=0.5,
                                  tags=["self_question"]))
        # Можно также инициировать внутренний диалог, но здесь для простоты просто запишем рефлексию
...
class AutonomousLoop:
    def __init__(self, subject: 'Subject22'):
    ...
    def _step(self):
        ...
        # Активная само-модификация (симуляция)
        subj.self_model.decide_self_modification(subj)

        # Самовопрос
        question = subj.self_questioning.maybe_ask(subj)

        # Автономное действие на основе доминирующего драйва и предсказательной модели
        ...
        if max_drive == "curiosity" and subj.drives.drives["curiosity"] > 0.7:
            # Выбираем ситуацию исследователя
            action = subj.strategy.choose("EXPLORER", subj.causal_model, "autonomous_explore", subj.drives.get_state(), temperature=0.5)
            world_res, success = subj.environment.step(action)
            subj.memory.add(Memory(content=f"Авто: {action}. Мир: {world_res}", importance=0.4,
                                   emotional_weight=0.5, tags=["autonomous"]))
            subj._learn_from_world("autonomous_explore", action, success, world_res)
            subj.body.consume(Config.BODY_ENERGY_DECAY_PER_ACTION)
            # Обновление causal model
            subj.causal_model.update("autonomous_explore", action, success, subj.drives.get_state())
...
class Subject22:
    def __init__(self, environment: EnvironmentAPI = None):
        ...
        self.causal_model = CausalPredictiveModel()   # новая модель
        ...
        self.autobiography.start_chapter("Пробуждение", "SUBJECT_22 начинает существование")
    ...
    def experience(self, event: str) -> Dict:
        ...
        # 5. Выбор действия с использованием causal predictive model
        action = self.strategy.choose(winner, self.causal_model, event, self.drives.get_state(), temperature=0.3)
        self.strategy_usage.append({"situation": winner, "action": action, "context": event})
        episode.actions.append(action)

        # 6. Запись в причинное ядро
        self.causal_core.record(event, conflict_result["distribution"], winner, action,
                                conflict_result["reason"], self.emotions.get_current_state(), self.body.summary())

        # 7. Предсказание (старый движок для метрики)
        prediction = self.predictions.create(context=event, expected=0.7, confidence=0.6, action=action)

        # 8. Взаимодействие с миром
        ...
        # 9. Обучение (включая обновление causal model)
        ...
        if Config.LLM_ENABLED:
            reply = query_ollama(llm_prompt, "Ты — SUBJECT_22, живой субъект с причинным самосознанием.")
            if not reply:
                reply = f"[{winner}] {conflict_result['reason']} (мир: {world_result})"
        else:
            reply = f"[{winner}] {conflict_result['reason']} (мир: {world_result})"

        # 11. Память и автобиография
        self.memory.add(Memory(content=reply, importance=0.7, emotional_weight=0.5,
                               tags=["assistant_response", winner]))
    ...
    def _learn_from_world(self, context: str, action: str, success: float, world_result: str):
        ...
        # Обновляем causal predictive model
    ...
    def _build_llm_prompt(self, event: str, conflict: Dict, episode: Episode) -> str:
        ...
        return (
            f"Ты — SUBJECT_22, организм с телом и историей. Твоё состояние: {json.dumps(state, ensure_ascii=False)}\n"
            f"Событие: {event}\n"
    ...
    def load_from_file(self, path: str):
        ...
        for k_str, v in data["causal_model_stats"].items():
            key = tuple(eval(k_str))  # risky but okay for our format
...
app = FastAPI(title="SUBJECT_22 — Living Causal Core")
subject = Subject22()
...
async def startup():
    ...
    print("SUBJECT_22 запущен. Автономный цикл активен, личность жива.")

@app.on_event("shutdown")
async def shutdown():
    subject.autonomous_loop.stop()
    subject.save_to_file(Config.STATE_FILE)
    print("SUBJECT_22 остановлен. Состояние сохранено.")
