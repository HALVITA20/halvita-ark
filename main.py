...
class HALVITA:
    ...
    def _call(self, prompt: str) -> str:
        try:
            resp = ollama.chat(model=MODEL, messages=self.history + [{"role": "user", "content": prompt}])
            return resp["message"]["content"]
        except Exception as e:
            return f"[Ошибка: {e}]"

    def _scan(self, text: str) -> dict:
        return {m: 1 if re.search(p, text, re.IGNORECASE) else 0 for m, p in MARKER_PATTERNS.items()}
