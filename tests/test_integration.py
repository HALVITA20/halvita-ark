...
class TestIntegration(unittest.TestCase):
    ...
    def test_metric_calculator(self):
        # Запускаем metric_calculator на тестовом файле
        result = subprocess.run(
            ['python', 'code/tools/metric_calculator.py', '--input', str(self.session_path)],
            capture_output=True,
            text=True
        )
    ...
    def test_anomaly_classifier(self):
        # Проверяем классификатор аномалий
        result = subprocess.run(
            ['python', 'code/tools/anomaly_classifier.py', str(self.session_path)],
            capture_output=True,
            text=True
        )
        self.assertIn("none", result.stdout)  # в тестовой сессии нет аномалий
